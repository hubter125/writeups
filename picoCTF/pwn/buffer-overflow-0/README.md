...
* **Objective:** Any payload that induces an unhandled memory violation (such as corrupting saved frame pointers or hijacking the saved return address to an unmapped region) triggers the signal handler and prints the flag.
...
| `Lower Memory` | **`buf2` Buffer** | 16 bytes |

Writing beyond 16 bytes spills into stack metadata:
1. **16 bytes:** Fills `buf2` completely.
2. **Padding + Saved EBP (4 bytes):** Overwrites compiler padding and overwrites the saved base pointer.
3. **The 21st Byte (`\0`):** Because `strcpy()` appends an implicit null byte (`\0`), a 20-character input actually writes 21 bytes, corrupting the least significant byte of the saved return address (`EIP`) and triggering `SIGSEGV` when `vuln()` returns.
